...
>>>>>>> 946a16bec46f657abc3a8b0d9a55992fb30acc56
  ...
  show do
    attributes_table do
      ...
      row :customer
      row :due_date do |obj|
        obj.created_at + obj.due_date.to_i.day
      end
      row :created_at
    ...
    panel "Products" do
      table_for invoice.invoice_items do
        ...
        column "Net value", :value do |product|
          div :class => "price" do
            number_to_currency product.value, :unit => "&pound;"
          end
            end
          column "VAT Value", :value do |obj|
            div :class => "price" do
          obj.vat? ? (number_to_currency obj.value/5, :unit => "&pound;" ) : (number_to_currency 0, :unit => "&pound;" )
              end
        end
        column "Gross", :value do |obj|
          div :class => "price" do
          obj.vat? ? (number_to_currency obj.value*6/5, :unit => "&pound;" ) : (number_to_currency obj.value, :unit => "&pound;")
        end
           end
        column :created_at
